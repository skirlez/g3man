...
public partial class MainWindow {
	...
	private void SetupAboutPage(Box page) {
		...
		Label versionLabel = Label.New($"Version {ProgramConstants.VERSION}");
		...
		void setUpdateFoundText(string version) {
			updateFoundLabel.SetMarkup(
				...
				+ $"\n(Latest is {version}, you are on {ProgramConstants.VERSION})"
		...
		(string? version) => {
			UI.RunOnMainThreadEventually(() => {
				...
				else if (version != ProgramConstants.VERSION.ToString()) {

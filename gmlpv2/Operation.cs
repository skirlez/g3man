...
public class LineState(string line) {
	...
	//$"{before}{beforeLast}{lineToReinsert}{conditions}{afterElseIf}{afterElseResult}{after}{afterLast}";
}
...
public abstract class Operation(string name, LuaValueType[] additionalTypes) {
	...
	public static void AcquiantAll(LuaTable table) {
		foreach (Operation operation in All) {
			operation.Acquaint(table);
		}
	}
	
	private readonly LuaValueType[] additionalTypes = additionalTypes;

	public abstract ValueTask<int> Perform(FileRecord record, CodeFile file, LuaFunctionExecutionContext context, CancellationToken ct);
	
	protected void AddWrappedOperation(LuaTable table, Func<FileRecord, CodeFile, LuaFunctionExecutionContext, CancellationToken, ValueTask<int>> func) {
		table[name] = new LuaFunction(name, (context, ct) => {
			...
			return func(record, file, context, ct);
		});
	}
	...
	private void Acquaint(LuaTable table) {
		AddWrappedOperation(table, this.Perform);
	}
}
...
public class WriteLineOperation(string name, Action<LineState, string> apply) : 
	...
	public override ValueTask<int> Perform(FileRecord record, CodeFile file, LuaFunctionExecutionContext context, CancellationToken ct) {
